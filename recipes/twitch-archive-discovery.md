...
TwitchのVOD URLをユーザーに探させず、assistant側でchannelを特定し、Twitchのarchive一覧から候補を発見し、意味的に候補を絞る。その後、選ばれたVODをprivate local-control経由でユーザーPCへ直接保存できる状態にする。
...
強い候補が複数ある場合だけ候補を示す。一般に表示名、公式リンク、他SNSからTwitch handleを解決できるならユーザーへhandleを聞き返さない。

## Local Discovery request

private `AIUse-local-control` が利用できる場合、`research-requests/*.json` へ `twitch_archive_search` を送れる。
...
Twitch/yt-dlp側のarchives filterが空になる既知ケースへ備え、0件なら `filter=all` へfallbackする。fallbackした場合はmanifestへ記録する。

## Semantic shortlist

ローカルbackendの `lexical_score` は補助情報であり、最終判定ではない。
...
ユーザーが明確にDLを依頼した候補だけ `download_local` requestへ渡す。
...
- local-control側でrelease assetを必要時に取得してcacheする。
- defaultはhighest available quality。
- `quality`指定がある場合だけCLIへ渡す。
- output collisionは対話promptを出さないようlocal-control側でunique filenameを作る。
